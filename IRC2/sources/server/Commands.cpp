...
#include "../../headers/channel/Channel.hpp"
	...
	void	Server::quitChan(Client &cl, Channel &chan) {
		...
		lp_channel(cl, chan, "PART " + chan.getName() + " :" + cl.getRequest().at("NICK").at(0) + " left the channel1.");
	}
...
void	Server::join_existing_channel(Channel *channel_tojoin, Client &client)
{
	...
	std::cout << "existing channel tojoin infos - channel_name - password - client :  " << channel_tojoin->getName() << "," << channel_tojoin->getChannelPass() << "," << client_name << std::endl;
}
